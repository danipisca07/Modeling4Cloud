...
router.route('/upload').post(upload.single('data'), function (req, res) {
    fs.rename(UPLOAD_PATH+req.file.filename, UPLOAD_PATH+req.file.originalname, function(err){
        ...
        exec('tail -n +2 '+ UPLOAD_PATH+req.file.originalname +' |  sed \'/,,/d\' | sed \'/,SA,/d\' | mongoimport -h localhost:27017 -d pings -c pings --type csv --columnsHaveTypes --fields "provider.string\(\),from_zone.string\(\),to_zone.string\(\),from_host.string\(\),to_host.string\(\),icmp_seq.int32\(\),ttl.int32\(\),time.double\(\),timestamp.date\(2006-01-02T15:04:05-00:00\)"', (err, stdout, stderr) => {
...
router.route('/pings/query/avgOfEveryPingOfSelectedDate').get(async (req, res, next) => {
    ...
    Ping.aggregate()
        ...
        .addFields({
            sameRegion: {$cmp:["$_id.from_zone", "$_id.to_zone"]}
        })
        .match({
...
router.route('/bandwidths/query/avgOfSelectedDate').get(async (req, res, next) => {
    ...
    Bandwidth.aggregate()
        .project({sameRegion: {$cmp: ['$from_zone', '$to_zone']}, provider: "$provider", from_zone:"$from_zone", to_zone:"$to_zone", bandwidth: "$bandwidth", timestamp: "$timestamp"})
...
router.route('/pings/query/avgOfEveryDayOfSelectedYear').get(async (req, res, next) => {
    ...
    PingDayAvg.aggregate()
        .addFields({year: {"$year":"$day"}, dayOfYear:{"$dayOfYear":"$day"}, sameRegion: {$cmp:["$from_zone", "$to_zone"]}})
